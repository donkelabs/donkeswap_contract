...
Because routers are stateless and do not hold token balances, they can be replaced safely and trustlessly, if necessary.
This may happen if more efficient smart contract patterns are discovered, or if additional functionality is desired. For
this reason, routers have _release numbers_, starting at `01`. This is currently recommended release, `02`.

# Code

[`DonkeV2Router02.sol`](../../../../contracts/periphery/DonkeV2Router02.sol)

# Address

`DonkeV2Router02` is deployed at `0x6e8D0B4EBe31C334D53ff7EB08722a4941049070` on the
Inevm [mainnet](https://seitrace.com/address/0x6e8D0B4EBe31C334D53ff7EB08722a4941049070?chain=pacific-1), and
the [testnet](https://seitrace.com/address/0xC5042FB122948FaBa027116157694c7439667EEd?chain=atlantic-2).
...
Returns the [canonical WETH address](https://blog.0xproject.com/canonical-weth-a9aa7d0279dd) on the
Inevm [mainnet](https://seitrace.com/token/0xE30feDd158A2e3b13e9badaeABaFc5516e95e8C7?chain=pacific-1), or
the [testnet](https://seitrace.com/address/0x3921eA6Cf927BE80211Bb57f19830700285b0AdA?chain=atlantic-2).
...
Removes liquidity from an ERC-20⇄WETTH pool and receive ETH without pre-approval, thanks
to [permit](pair-erc-20#permit).
...
Identical to [removeLiquidityETHWithPermit](#removeliquidityethwithpermit), but succeeds for tokens that take a fee on
transfer.
...
intermediate tokens to trade through (if, for example, a direct pair does not exist).
...
element of path must be [WETH](#weth), the last is the output token, and any intermediate elements represent
intermediate pairs to trade through (if, for example, a direct pair does not exist).
...
element of path is the input token, the last must be [WETH](#weth), and any intermediate elements represent intermediate
...
of path is the input token, the last must be [WETH](#weth), and any intermediate elements represent intermediate pairs
...
element of path must be [WETH](#weth), the last is the output token and any intermediate elements represent intermediate
pairs to trade through (if, for example, a direct pair does not exist).
...
```solidity
import '@donke/contracts/periphery/interfaces/IDonkeV2Router02.sol';
```

```solidity
pragma solidity >=0.6.2;

interface IDonkeV2Router01 {
    function factory() external pure returns (address);
...
interface IDonkeV2Router02 is IDonkeV2Router01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
