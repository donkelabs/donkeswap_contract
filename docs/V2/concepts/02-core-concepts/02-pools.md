...
Donke is unique in that it doesn’t use an order book to derive the price of an asset or to match buyers and sellers of tokens. Instead, Donke uses what are called Liquidity Pools.
...
Donke focuses on the strengths of Ethereum to reimagine token swaps from first principles.
...
Just how end-users can interact with the Donke protocol through the Interface (which in turn interacts with the underlying contracts), developers can interact directly with the smart contracts and integrate Donke functionality into their own applications without relying on intermediaries or needing permission.
